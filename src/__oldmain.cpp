...
int main ( int const argc , char const *const *const argv ) {
    if ( argc != 3 ) {
        std::cout << "Usage: " << argv [ 0 ] << " course-manifest-file plan-manifest-file\n";
        return 0;
    } else {
        ...
        for ( Plan plan : globalPlans ) {
            std::cout << "Plan \"" << plan.name << "\": " << plan.failureMessage ( ) << std::endl;
            std::cout << plan.semesterBreakdown ( ) << std::endl;
        }
        std::cout << "Press enter to view classes by semester and by their (human readable) name. Press Ctrl+C to exit.\n";
        std::cin.get();
        for ( Plan plan : globalPlans) {
...
std::istream &operator>>( std::istream &stream , Course &course ) {
    ...
    while ( !stream.eof ( ) && getline ( ) != "end" ) {
        ...
        if ( keyword == "name" ) {
            course.name = toParse.str ( ).substr ( toParse.str ( ).find ( keyword ) + keyword.size ( ) + 1 );
            ...
            course.desc = toParse.str ( ).substr ( toParse.str ( ).find ( keyword ) + keyword.size ( ) + 1 );
        }
    }
...
std::istream &operator>>( std::istream &stream , Plan &plan ) {
    ...
    while ( !stream.eof ( ) && getline ( ) != "end" ) {
        ...
        if ( keyword == "name" ) {
            plan.name = toParse.str ( ).substr ( toParse.str ( ).find ( keyword ) + keyword.size ( ) + 1 );
            ...
            if ( course == nullptr ) {
                ...
                for ( Requisite requisite : course->requisites ) {
                    if ( requisite.acceptable.empty ( ) ) {
                        //std::cout << "Error: the one of the requisites for " + currentClass + " is blank.\n";
                        continue;
                    }
                    bool matched = false;
                    if ( requisite.allowPreviously ) {
                        ...
                        if ( matched ) break;
                        // if we did not match, go to previous semesters and check there
                        else for ( int j = 0; j < i; j++ ) {
                            for ( std::string clazz : semesters [ j ] ) {
                                Course *previous = getCourseFromGlobal ( clazz );
                                if ( !previous ) unsure = true;
                                else {
                                    if ( requisite.containsCourse ( *previous ) ) {
                                        matched = true;
                                        break;
                                    }
                                }
                    ...
                    if ( matched ) continue;
                        ...
                        for ( std::string concurrent : semesters [ i ] ) {
                            if ( concurrent == currentClass ) continue;
                            else {
                                Course *concurrentCourse = getCourseFromGlobal ( concurrent );
                                if ( !concurrentCourse ) unsure = true;
                                else {
                                    if ( requisite.containsCourse ( *concurrentCourse ) ) {
                                        matched = true;
                                        break;
                                    }
                                }
                    ...
                    if ( !matched ) {
                        return "Failed to match requisite of course " + currentClass + ": " + requisite.str ( );
                    }
                }
            }
        }
    }
    return !unsure ? "No failure." : "No failure found, but some courses are not registered.";
}
...
std::string Plan::semesterBreakdown ( ) {
    ...
    static char const *const semesterNames [ ] = { "Incoming AP", "Fall - 21", "Spring - 22", "Fall - 22", "Spring - 23", "Fall - 23", "Spring - 24", "Fall - 24", "Spring - 25" };
    ...
    result << "The Plan: \n";
    for ( int i = 0; i < 9; i++ ) {
        result << std::left << std::setw ( std::strlen ( semesterNames [ i ] ) + 5 ) << ( std::string ( semesterNames [ i ] ) ) << "|";
    } result << "\n";
    std::vector<std::string> courseNames [ 9 ] = { incomingAPCredits, semesters [ 0 ], semesters [ 1 ], semesters [ 2 ], semesters [ 3 ], semesters [ 4 ], semesters [ 5 ], semesters [ 6 ], semesters [ 7 ] };
    auto checkEmpty = [ & ] ( ) {
        bool allEmpty = false;
        for ( int i = 0; i < 9; i++ ) {
    ...
    while ( checkEmpty ( ) ) {
        for ( int i = 0; i < 9; i++ ) {
            if ( courseNames [ i ].empty ( ) ) {
                result << std::right << std::setw ( std::strlen ( semesterNames [ i ] ) + 6 ) << "|";
            } else {
                std::stringstream row;
                row << std::left << std::setw ( std::strlen ( semesterNames [ i ] ) + 1 ) << courseNames [ i ].back ( );
                row << std::right << std::setw ( 4 ) << ( getCourseFromGlobal ( courseNames [ i ].back ( ) ) ? getCourseFromGlobal ( courseNames [ i ].back ( ) )->hours : 0.0 );
                row << std::setw ( 1 ) << "|";
                result << std::left << std::setw ( std::strlen ( semesterNames [ i ] ) + 6 ) << row.str ( );
    ...
    for ( int j = 0; j < 3; j++ ) {
        for ( int i = 0; i < 9; i++ ) {
            std::stringstream row;
            row << std::left << std::setw ( std::strlen ( semesterNames [ i ] ) + 1 - ( j == 3 ) ) << creditNames [ j ];
            double *hours = totalHours ( i - 1 );
            row << std::right << std::setw ( 4 + ( j == 3 ) ) << ( hours [ j ] );
            delete [ ] hours;
            row << std::setw ( 1 ) << "|";
            result << std::left << std::setw ( std::strlen ( semesterNames [ i ] ) + 6 ) << row.str ( );
        }
...
double *Plan::totalHours ( int semester ) {
    double *result = new double [ 3 ] { 0.0, 0.0, 0.0 }; // graduate hours count as honors, so graduate + honors doesn't make sense
    if ( semester == -1 ) {
